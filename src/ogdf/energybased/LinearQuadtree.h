...
namespace ogdf
{
    ...
    public:
        ...
        struct is_fence_condition_functor
        {
            ...
            inline bool operator()(NodeID u)
            {
                return tree.isFence(u);
            }
        };
        ...
        struct StoreWSPairFunctor
        {
            ...
            inline void operator()(NodeID a, NodeID b)
            {
                tree.addWSPD(a, b);
            }
        };

        StoreWSPairFunctor inline StoreWSPairFunction()
        {
            return StoreWSPairFunctor(*this);
        }
        ...
        inline float pointX(PointID point) const
        {
            return m_pointXPos[point];
        }

        inline float pointY(PointID point) const
        {
            return m_pointYPos[point];
        }

        inline float pointSize(PointID point) const
        {
            return m_pointSize[point];
        }

        inline float* pointX() const
        {
            return m_pointXPos;
        }

        inline float* pointY() const
        {
            return m_pointYPos;
        }
        ...
        inline float nodeX(NodeID node) const
        {
            return m_nodeXPos[node];
        }

        inline void setNodeX(NodeID node, float x)
        {
            m_nodeXPos[node] = x;
        }

        inline float nodeY(NodeID node) const
        {
            return m_nodeYPos[node];
        }

        inline void setNodeY(NodeID node, float y)
        {
            m_nodeYPos[node] = y;
        }

        inline float nodeSize(NodeID node) const
        {
            return m_nodeSize[node];
        }

        inline void setNodeSize(NodeID node, float size)
        {
            m_nodeSize[node] = size;
        }
        ...
        inline NodeID firstInnerNode() const
        {
            return m_firstInner;
        }

        inline __uint32 numberOfInnerNodes() const
        {
            return m_numInnerNodes;
        }

        inline NodeID firstLeaf() const
        {
            return m_firstLeaf;
        }

        inline __uint32 numberOfLeaves() const
        {
            return m_numLeaves;
        }


        __uint32 numberOfWSP() const
        {
            return m_numWSP;
        }

        __uint32 numberOfDirectPairs() const
        {
            return m_numNotWSP;
        }

        __uint32 numberOfDirectNodes() const
        {
            return m_numDirectNodes;
        }

        inline NodeID directNode(__uint32 i) const
        {
            return m_directNodes[i];
        }

        inline NodeID directNodeA(__uint32 i) const
        {
            return m_notWspd[i].a;
        }

        inline NodeID directNodeB(__uint32 i) const
        {
            return m_notWspd[i].b;
        }
