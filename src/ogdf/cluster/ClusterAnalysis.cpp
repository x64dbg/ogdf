...
int ClusterAnalysis::outerActive(cluster c)
{
    return (*m_oanum)[c];
}
int ClusterAnalysis::innerActive(cluster c)
{
    return (*m_ianum)[c];
}
int ClusterAnalysis::numberOfBags(cluster c)
{
    return (*m_bags)[c];
}
...
int ClusterAnalysis::bagIndex(node v, cluster c)
{
    return ((*m_bagindex[v])[c]);
}
