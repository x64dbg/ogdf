...
public:
    ...
    face left(adjEntry adj)
    {
        return m_pEmbedding->leftFace(adj);
    }

    // returns face right of adj
    face right(adjEntry adj)
    {
        return m_pEmbedding->rightFace(adj);
    }

    // if v = c_i, returns face right of c_i -> c_i+1
    face right(node v)
    {
        return left(m_nextSucc[v]);
    }

    // if v = c_i, returns face left of c_i -> c_i-1
    face left(node v)
    {
        return right(m_prevPred[v]);
    }
