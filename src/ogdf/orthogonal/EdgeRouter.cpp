...
//and generalizations for horizontal edges and set bend type accordingly
{
    ...
    if (infos[v].has_gen(odNorth))//gen at left side
    {
        ...
        while (l_it.valid())
        {
            ...
            {
                m_abends[outEntry(infos[v], odNorth, pos)] = prob_b1l;    //numr??
                infos[v].inc_E_hook(odNorth, odWest);
            }
            else
            {
                m_abends[outEntry(infos[v], odNorth, pos)] = prob_b2l;
        ...
        while (l_it.valid())
        {
            ...
            {
                m_abends[outEntry(infos[v],odNorth, pos)] = prob_b2r;
...
void EdgeRouter::initialize_node_info(node v, int sep)
{
    ...
    if (infos[v].has_gen(odEast))
    {
        ...
        dval = min(dsep,sep);
        OGDF_ASSERT(dval > 0);
        infos[v].set_delta(odEast,odNorth,dval);
        infos[v].set_eps(odEast,odNorth,int(floor(Cconst*dval)));
        //top side
        dsep = ( (re+Cconst == 0) ? sep : int(floor((double)(infos[v].node_xsize()) / (2*(re+Cconst)))) );
        dval = min(dsep, sep);
        OGDF_ASSERT( dval > 0);
        infos[v].set_delta(odEast,odSouth,dval);
        infos[v].set_eps(odEast,odSouth,int(floor(Cconst*dval)));
    }//if top
    else
    {
    ...
    if (infos[v].has_gen(odSouth))
    {
        ...
        dval = min(dsep,sep);
        OGDF_ASSERT( dval > 0);
        infos[v].set_delta(odSouth,odEast,dval);
        infos[v].set_eps(odSouth,odEast,int(floor(Cconst*dval)));
        //top side
        dsep = ( (re+Cconst == 0) ? sep : int(floor((double)(infos[v].node_ysize()) / (2*(re+Cconst)))) );
        dval = min(dsep, sep);
        infos[v].set_delta(odSouth,odWest,dval);
        infos[v].set_eps(odSouth,odWest,int(floor(Cconst*dval)));
    }//if right
    else
    {
    ...
    if (infos[v].has_gen(odWest))
    {
        ...
        dval = min(dsep,sep);
        OGDF_ASSERT( dval > 0);
        infos[v].set_delta(odWest,odSouth,dval);
        infos[v].set_eps(odWest,odSouth,int(floor(Cconst*dval)));
        //top side
        dsep = ( (re+Cconst == 0) ? sep : int(floor((double)(infos[v].node_xsize()) / (2*(re+Cconst)))) );
        dval = min(dsep, sep);
        infos[v].set_delta(odWest,odNorth,dval);
        infos[v].set_eps(odWest,odNorth,int(floor(Cconst*dval)));
    }//if bottom
    else
    {
